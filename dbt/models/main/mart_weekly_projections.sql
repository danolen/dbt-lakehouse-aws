...
select
    ...
    -- Prefer dedicated Mon–Thu Hittertron $ (#210); fall back to weekly $ MT.
    cast(
    ...
    -- Mon–Thu hitter components from dedicated scrape (#210).
    cast(nullif(mt.num_g, '') as int) as mt_num_g,
    cast(nullif(mt.hg, '') as int) as mt_home_games,
    cast(nullif(mt.away, '') as int) as mt_away_games,
    cast(nullif(mt.vr, '') as int) as mt_vs_rhp,
    cast(nullif(mt.vl, '') as int) as mt_vs_lhp,
    nullif(mt.opp, '') as mt_opp,
    nullif(mt.sp, '') as mt_sp,
    cast(nullif(mt.g, '') as double) as mt_hit_g,
    cast(nullif(mt.pa, '') as double) as mt_pa,
    cast(nullif(mt.ab, '') as double) as mt_ab,
    cast(nullif(mt.h, '') as double) as mt_hits,
    cast(nullif(mt.r, '') as double) as mt_r,
    cast(nullif(mt.hr, '') as double) as mt_hr,
    cast(nullif(mt.rbi, '') as double) as mt_rbi,
    cast(nullif(mt.sb, '') as double) as mt_sb,
    cast(nullif(mt.bb, '') as double) as mt_bb,
    ...
    -- True when a current weekly hitting / pitching / Mon–Thu row exists.
